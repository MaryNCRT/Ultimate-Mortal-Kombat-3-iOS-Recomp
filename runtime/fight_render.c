/*
 * fight_render.c -- the stage and the character, for the fight scene.
 *
 * **This is `runtime/demo.c`'s rendering half, lifted out verbatim.** Lines 62
 * to 993 of that file, moved rather than retyped, with a small public API
 * bolted on at the bottom. Everything it draws comes from formats this
 * repository specified and code it verified:
 *
 ...
 *     .scene      where each stage object goes    docs/SCENE-FORMAT.md
 *     .lighting   per-vertex light for the stage  docs/LIGHTING.md
 *     PVRTC       every texture                   docs/PVR-FORMAT.md
 *
 * No emulator, no engine binary, and **no asset shipped with this repository**
 * -- the caller passes the path to its own extracted `UMK3.app/res`.
 *
 * ## The duplication is deliberate and temporary
 *
 * `demo.c` still carries its own copy. Two executables now contain the same
 * four hundred lines, and that is worth fixing -- but not by editing a working
 * demo while trying to get a fight on screen. The right move later is for
 * demo.c to include this module and delete its copy; doing it the other way
 * round risks breaking the one thing in the repo that is known to render
 * correctly.
 *
 * **If you change something here, change it there too, until that is done.**
 *
 * ## One character, drawn twice
 *
 * The skinning buffers are single-instance, exactly as demo.c had them. The
 * fight scene poses, skins and draws one fighter, then poses, skins and draws
 * the other -- so two fighters cost two skinnings a frame and share every
 * buffer. That is why `fr_char_pose` and `fr_char_draw` are separate calls
 * rather than one: the pose has to land between them.
 */
...
/* ======================================================== the public surface
 *
 * Six calls. Everything above is demo.c's; everything here is the join.
 */

int fr_stage_load(const char *res_dir, const char *name)
{
    return stage_load(res_dir, name);
}
...
{
    return g_stage_reach;
}

int fr_char_load(const char *res_dir, const char *name)
{
    lime_light_init();
    return character_load(res_dir, name);
}

int fr_char_frames(void)
{
    return g_anim.num_frames;
}

/* Pose the skeleton between two animation frames and skin it. The result sits
 * in the shared buffers until the next call, which is why the draw has to
 * follow immediately. */
void fr_char_pose(int fa, int fb, float frac)
{
    LimePalette pal[LIME_MAX_BONES];

    if (g_anim.num_frames <= 0)
        return;
    if (fa < 0) fa = 0;
    if (fb < 0) fb = 0;
    if (fa >= g_anim.num_frames) fa = g_anim.num_frames - 1;
    if (fb >= g_anim.num_frames) fb = g_anim.num_frames - 1;
...
float fr_fov(void)
{
    return GAME_FOV_DEGREES;
}

float fr_player_scale(void)
{
    return PLAYER_TO_SCENE;
}

/* The character's bounding box in scene units, from whatever pose is built.
 * The fight scene needs it for two things the demo did not: the scale between
 * the engine's world and this one, and where the feet are. */
void fr_char_extent(float *lo, float *hi)
{
    long i;
    int  c;

    for (c = 0; c < 3; c++) { lo[c] = 1e30f; hi[c] = -1e30f; }
    for (i = 0; i < g_tri_count * 3; i++)
        for (c = 0; c < 3; c++) {
            float v = g_vb[i * 3 + c];
            if (v < lo[c]) lo[c] = v;
            if (v > hi[c]) hi[c] = v;
